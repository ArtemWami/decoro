    ...
    try {
        await Promise.all([
            queryInterface.addConstraint(tableName, {
                transaction,
                fields: ['palette_location_id'],
                type: 'foreign key',
                name: `fkey_${tableName}_palette_location_id`,
                onDelete: 'cascade',
                onUpdate: 'cascade',
                references: {
                    table: 'palette_locations',
...
const down = async (queryInterface) => {
    ...
    try {
        await Promise.all([
            queryInterface.removeConstraint(tableName, `fkey_${tableName}_palette_location_id`, {
                transaction,
            }),
